...
// Fútbol: 1 / X / 2   →  local / empate / visitante
// Béisbol: solo Local / Visitante (no hay empate en MLB)
const FUTBOL_OPCIONES  = ['local', 'empate', 'visitante'] as const;
const BEISBOL_OPCIONES = ['local', 'visitante']            as const;

const LABEL_FUTBOL:  Record<string, string> = { local: '1',      empate: 'X',          visitante: '2'        };
const LABEL_BEISBOL: Record<string, string> = { local: 'Local',  visitante: 'Visitante'                      };
...
export default function QuinielaDetailScreen() {
  ...
  // ─── ¿Es quiniela de béisbol? ─────────────────────────────────────────────
  const esBeisbol = quiniela?.deporte === 'beisbol';
  const OPCIONES  = esBeisbol ? BEISBOL_OPCIONES : FUTBOL_OPCIONES;
  const LABEL     = esBeisbol ? LABEL_BEISBOL    : LABEL_FUTBOL;
  // Etiqueta de puntos: "Carreras" para béisbol, "Goles" para fútbol
  const labelPuntos = esBeisbol ? 'Carreras' : 'Goles';

  const compartirImagen = useCallback(async () => {
    if (!quiniela || partidos.length === 0) return;
    if (Platform.OS === 'web') {
      setSharing(true); setShowCard(true);
      setTimeout(async () => {
        try {
          const dataUrl = await captureView(cardViewRef);
          const a = document.createElement('a');
          a.href = dataUrl; a.download = `${quiniela?.titulo ?? 'quiniela'}.png`; a.click();
        } catch (e: any) {
          Alert.alert('Error', `No se pudo generar la imagen.\n\n${e?.message}`);
        } finally { setShowCard(false); setSharing(false); }
      }, 600);
      return;
    }
    const canShare = await Sharing.isAvailableAsync();
    if (!canShare) { Alert.alert('No disponible', 'Tu dispositivo no soporta compartir archivos.'); return; }
    setSharing(true); setShowCard(true);
    setTimeout(async () => {
      try {
        const uri = await captureView(cardViewRef);
        await Sharing.shareAsync(uri, { mimeType: 'image/png', dialogTitle: quiniela?.titulo ?? 'Compartir quiniela', UTI: 'public.png' });
      } catch (e: any) {
        const msg = String(e?.message ?? '');
        if (!msg.includes('cancel') && !msg.includes('dismiss')) Alert.alert('Error', `No se pudo generar la imagen.\n\n${msg}`);
      } finally { setShowCard(false); setSharing(false); }
    }, 800);
  ...
  const reloadPartidos = useCallback(async () => {
    const { data: pts } = await supabase.from('partidos').select('*').eq('quiniela_id', id!).order('orden', { ascending: true });
    if (pts) setPartidos(pts);
    if (myUserId.current) {
      const { data: part } = await supabase.from('participaciones').select('id, aciertos, estado, premio_ganado, monto_pagado').eq('quiniela_id', id!).eq('user_id', myUserId.current).single();
  ...
  const load = useCallback(async () => {
    try {
      ...
      const { data: part } = await supabase.from('participaciones').select('id, aciertos, estado, premio_ganado, monto_pagado').eq('quiniela_id', id!).eq('user_id', user.id).single();
      setMiPart(part ?? null);
      if (part) {
      ...
      >
        ...
        {esBeisbol && (
          <View style={s.deporteBadge}>
            <Text style={s.deporteBadgeTxt}>⚾ Quiniela de Béisbol MLB · Sin empate</Text>
          </View>
        )}
          ...
          {(['picks', 'ranking'] as const).map(t => (
            <TouchableOpacity key={t} style={[s.tab, tabActivo === t && s.tabActivo]} onPress={() => setTabActivo(t)}>
              <Text style={[s.tabTxt, tabActivo === t && s.tabTxtActivo]}>
                {t === 'picks' ? (esBeisbol ? '⚾ MIS PICKS' : '🎯 MIS PICKS') : '🏆 RANKING'}
              </Text>
            </TouchableOpacity>
          ))}
        </View>
            ...
            {partidos.map((p, i) => {
              ...
              return (
                <View key={p.id} style={[s.partidoCard, { borderLeftColor: neon, shadowColor: neon }]}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {tieneRes && (
                        <Text style={[s.resultadoTxt, { color: '#606060' }]}>
                          Ganó: <Text style={{ color: neon }}>{RES_LABEL[p.resultado]}</Text>
                          {/* Si tiene marcador, mostrarlo con label correcto */}
                          {p.goles_local != null && p.goles_visitante != null
                            ? `  (${labelPuntos}: ${p.goles_local} - ${p.goles_visitante})`
                            : ''}
                        </Text>
                      )}
                    </View>
                  ...
                  {/* Opciones: 1/X/2 para fútbol, Local/Visitante para béisbol */}
                  <View style={s.opcionesRow}>
                    {(OPCIONES as readonly string[]).map(op => {
...
const s = StyleSheet.create({
  ...
  // Badge deporte
  deporteBadge:       { backgroundColor: 'rgba(232,83,74,0.10)', borderRadius: 10, borderWidth: 1, borderColor: 'rgba(232,83,74,0.4)', paddingHorizontal: 12, paddingVertical: 6, alignSelf: 'flex-start', marginBottom: 12 },
  deporteBadgeTxt:    { color: '#E8534A', fontSize: 12, fontWeight: '700' },
  // Resumen
