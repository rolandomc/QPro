-- Notificaciones de resultado completas:
-- - Ganadores: incluye lugar y monto
-- - Perdedores: incluye lugar final y mensaje de resultado
...
BEGIN
  ...
  UPDATE participaciones p
    ...
    FROM (
      SELECT ganador_id, monto, posicion
      FROM unnest(p_ganador_ids, p_montos_ganador) WITH ORDINALITY AS t(ganador_id, monto, posicion)
    ) g
  ...
  SELECT
    ...
    'Terminaste en ' ||
      CASE w.posicion
        ...
        WHEN 2 THEN '2do'
        WHEN 3 THEN '3er'
        ELSE w.posicion::text || 'to'
      END ||
      ' lugar (posición general #' || r.pos::text || ') con ' ||
      COALESCE(p.aciertos, 0) || ' acierto' || CASE WHEN COALESCE(p.aciertos, 0) = 1 THEN '' ELSE 's' END ||
      '. Premio: $' || TO_CHAR(w.monto, 'FM999G999G999G990D00') || '.',
    false,
    p.id::text,
    'participacion_premio'
  FROM participaciones p
  JOIN winners w ON w.ganador_id = p.id
  ...
  SELECT
    ...
    'Terminaste en la posición #' || r.pos::text || ' con ' ||
      COALESCE(p.aciertos, 0) || ' acierto' || CASE WHEN COALESCE(p.aciertos, 0) = 1 THEN '' ELSE 's' END ||
