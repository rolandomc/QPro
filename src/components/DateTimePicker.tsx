...
export default function DateTimePicker({ visible, initialDate, onConfirm, onCancel }: DateTimePickerProps) {
  ...
  const handleConfirm = () => {
    const d = new Date(selectedYear, monthIdx, safeDayIdx + 1, hourIdx, minIdx, 0);
    onConfirm(d);
  };

  const colDay = 52; const colMonth = 64; const colYear = 72;
  const colHour = 52; const colMin = 52;
