...
function useCountdown(targetDate: string | null) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setTiempo({ dias, horas, minutos, segundos, pasado: false });
    };
    tick();
...
export default function EmptyQuinielas() {
  ...
  const handleNotificacion = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permisos denegados', 'Activa las notificaciones en Ajustes para recibir alertas.');
        return;
      }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <TouchableOpacity
        ...
        disabled={notifActiva || loadingNotif}
      >
...
const styles = StyleSheet.create({
  ...
  countdownNum:        { color: '#FFF', fontSize: 36, fontWeight: 'bold', fontVariant: ['tabular-nums'] },
