...
const NEON_POS = (pos: number) => pos === 1 ? '#FFD700' : pos === 2 ? '#C0C0C0' : pos === 3 ? '#CD7F32' : '#9B59B6';

export default function ResultCard({ quiniela, participacion, selecciones, modo, ganador, posicion, totalJugadores }: Props) {
  ...
  const pendientes = sels.length - conRes.length;
    ...
    <View style={[c.card, { borderColor: neonBorder, shadowColor: neonShadow }]}>
      ...
      <TouchableOpacity onPress={toggle} activeOpacity={0.8} style={c.headerTouch}>
        ...
        <View style={c.row}>
          ...
          {modo === 'historial' ? (
            <View style={c.aciBox}>
              <Text style={[c.aciNum, { color: pct >= 70 ? '#2ECC71' : pct >= 40 ? '#F39C12' : '#E91E63' }]}>{aciertos}</Text>
              <Text style={c.aciDen}>/{conRes.length}</Text>
            </View>
          ) : (
          ...
          <View style={c.histMeta}>
            ...
            {posicion != null && (
              <View style={[c.posBadge, { borderColor: NEON_POS(posicion) }]}>
                <Text style={[c.posNum, { color: NEON_POS(posicion), textShadowColor: NEON_POS(posicion), textShadowRadius: 6 }]}>
                  #{posicion}
                </Text>
            ...
            <Text style={c.pctTxt}>{pct}% de acierto</Text>

            {/* Ganador de la quiniela */}
            {ganador && (
              <View style={c.ganadorRow}>
                <Text style={c.ganadorLbl}>🏆 Ganó: </Text>
                <Text style={[c.ganadorNombre, { color: '#FFD700', textShadowColor: '#FFD700', textShadowRadius: 6 }]}>
                  {ganador.username}
                </Text>
      ...
      {expandido && (
        <View style={c.desglose}>
          ...
          {sels.map((s, i) => {
            ...
            return (
              <View key={s.partido_id} style={[
                c.partidoRow,
                { borderLeftColor: neonPart, borderLeftWidth: 2 },
              ]}>
...
const c = StyleSheet.create({
  ...
  pctTxt:    { color: '#404040', fontSize: 10, letterSpacing: 1 },
  ...
  desglose:      { borderTopWidth: 1, borderTopColor: '#1E2330', paddingHorizontal: 14, paddingTop: 12, paddingBottom: 6, gap: 8 },
  ...
  footerTxt: { fontSize: 10, fontWeight: 'bold', letterSpacing: 2,
               textShadowRadius: 6 },
});
