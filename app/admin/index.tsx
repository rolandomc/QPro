...
export default function AdminDashboardScreen() {
  ...
  const [quinielaSeleccionada, setQuinielaSeleccionada] = useState<string>('');
  ...
  const handleVerUsuarios = async (quinielaId: string, titulo: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          id, estado, monto_pagado, aciertos, created_at,
          profiles(username)
        `)
  ...
  // --- ACCION: Cerrar Apuestas ---
  const handleCerrarApuestas = (quinielaId: string, titulo: string) => {
    Alert.alert(
      'Cerrar Apuestas',
      `¿Cerrar la quiniela "${titulo}"?\n\nYa nadie podrá participar ni modificar sus selecciones.`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              await loadQuinielas();
              Alert.alert('✅ Listo', `Las apuestas de "${titulo}" han sido cerradas.`);
            } catch (e: any) {
              Alert.alert('Error', e.message);
            } finally {
  ...
  // --- ACCION: Cancelar Quiniela ---
  const handleCancelar = (quinielaId: string, titulo: string) => {
    Alert.alert(
      '⚠️ Cancelar Quiniela',
      `¿Cancelar "${titulo}" definitivamente?\n\nSe marcará como finalizada. En una versión futura esto reembolsará el saldo a los usuarios.`,
      [
  ...
  const totalUsuarios = quinielas.reduce((acc, q) => acc + (q.partidos?.[0]?.count ?? 0), 0);
          ...
          <View key={q.id} style={styles.card}>
            ...
            <View style={styles.cardActions}>
              ...
              <TouchableOpacity
                ...
                onPress={() => handleVerUsuarios(q.id, q.titulo)}
              >
                <Text style={styles.actionText}>👥 Usuarios</Text>
              </TouchableOpacity>

              {/* Cerrar Apuestas — solo si está abierta */}
              <TouchableOpacity
                ...
                renderItem={({ item, index }) => (
                  <View style={styles.userRow}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.userMeta}>Aciertos: {item.aciertos ?? 0} • Estado: {item.estado}</Text>
                    </View>
